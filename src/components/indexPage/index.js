...
class IndexPage extends Component {
  ...
  drag(event) {
    ...
    this.setState({ leftValue: parseInt(style.getPropertyValue("left"), 10) - event.clientX, topValue: parseInt(style.getPropertyValue("top"), 10) - event.clientY })
  }
  drop(event) {
    let data = event.dataTransfer.getData("text");
    let dm = document.getElementById(data);
    ...
    let nodeCopy = dm.parentNode.id === "div1" ? dm : dm.cloneNode(true);
    nodeCopy.id = data;
    ...
    return (
      <div className="container-fluid p-0">
        ...
        <div className="features-list d-flex">
          ...
          <div id="div1" className="w-100 page-region" draggable="true" onDragStart={(event) => this.drag(event)} onDrop={(event) => this.drop(event)} onDragOver={(event) => this.allowDrop(event)}>
          </div>
        </div>
        {/* FEATURE SECTION END */}
            ...
            <div className="widget-category" key={widget.widgetName}>
              ...
              <ul>
                {widget.widgetImgs && widget.widgetImgs.map((img, i) =>
                  <li className="pl-0" id={widget.widgetName}><img src={img} id={widget.widgetName + i} draggable="true" onDragStart={(event) => this.drag(event)} /></li>
                )}
              </ul>
            </div>
          )}
